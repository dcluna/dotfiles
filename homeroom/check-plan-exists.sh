...
if [ "$PLAN_DOC_MODE" = "clean" ]; then
  # In clean mode: ensure no NEW plan docs were added on this branch
  # (docs already on the base branch are ignored)
...
MSG
  ...
  echo "Clean mode: no branch-added plan documents in git. PASS."
  exit 0
fi
