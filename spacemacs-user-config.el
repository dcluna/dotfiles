...
(defcustom dcl/protected-branches '("master" "main" "develop" "staging" "production")
  "Branches that `magit-push-matching' should never push."
  :type '(repeat string)
  :group 'magit)

(defun dcl/magit-push-matching (remote &optional args)
  "Like `magit-push-matching' but skips protected branches."
  (interactive (list (magit-read-remote "Push matching branches to" nil t)
                     (magit-push-arguments)))
  (let* ((local-branches (magit-list-local-branch-names))
         (remote-branches (magit-list-remote-branch-names remote))
         (remote-stripped (mapcar (lambda (b)
                                   (string-remove-prefix (concat remote "/") b))
                                 remote-branches))
         (matching (seq-intersection local-branches remote-stripped))
         (safe (seq-remove (lambda (b) (member b dcl/protected-branches)) matching))
         (skipped (seq-intersection matching dcl/protected-branches)))
    (when skipped
      (message "Skipping protected branches: %s" (string-join skipped ", ")))
    (if safe
        (progn
          (run-hooks 'magit-credential-hook)
          (magit-run-git-async "push" "-v" args remote
                               (mapcar (lambda (b) (format "refs/heads/%s:refs/heads/%s" b b))
                                       safe)))
      (user-error "No non-protected matching branches to push"))))

(advice-add 'magit-push-matching :override #'dcl/magit-push-matching)
...
(defvar dcl/vagrant-docker-container nil
  "Control docker mode for vagrant shell commands.
- nil: use `vagrant ssh' (default).
- A string: use that container name directly.
- t: prompt for a running container via `docker-container-read-name'.")
...
(defun dcl/vagrant-shell--docker-tramp-path (container rel-path)
  "Return a TRAMP path for CONTAINER at REL-PATH."
  (format "/docker:%s:%s" container rel-path))

(defun dcl/vagrant-shell--resolve-container ()
  "Return the docker container name to use, or nil.
If `dcl/vagrant-docker-container' is t, prompt via `docker-container-read-name'.
If it is a string, return it directly.  Otherwise return nil."
  (pcase dcl/vagrant-docker-container
    ('nil nil)
    ('t (docker-container-read-name))
    ((pred stringp) dcl/vagrant-docker-container)))

(defun dcl/vagrant-shell--run (backend-name)
  "Open a vagrant shell using BACKEND-NAME.
When `dcl/vagrant-docker-container' is non-nil, eshell opens via
TRAMP's docker method and terminal backends use `docker exec'
instead of `vagrant ssh'."
  (let ((vagrant-dir (dcl/vagrant-dir)))
    ...
    (let* ((rel-path (dcl/vagrant-relative-path))
           (container (dcl/vagrant-shell--resolve-container))
           (cmd (if container
                    (format "docker exec -it %s bash -c \"cd %s && exec \\$SHELL -l\""
                            (shell-quote-argument container)
                            (shell-quote-argument rel-path))
                  (format "vagrant ssh -- -t \"cd %s && exec \\$SHELL -l\""
                          (shell-quote-argument rel-path))))
      ...
      (if (and container (string= backend-name "eshell"))
          (funcall backend-fn (dcl/vagrant-shell--docker-tramp-path container rel-path))
...
(defun dcl/vagrant-shell--eshell (cmd-or-path)
  "Open an `eshell' buffer.
When `dcl/vagrant-docker-container' is set, CMD-OR-PATH is a TRAMP
docker path used as `default-directory'; otherwise it is a shell
command to execute."
  (let ((eshell-buffer-name (dcl/vagrant-shell--buffer-name "eshell")))
    (if dcl/vagrant-docker-container
        (let ((default-directory cmd-or-path))
          (eshell))
      (eshell)
      (goto-char (point-max))
      (eshell-kill-input)
...
(defvar llm-agent-keymap (make-sparse-keymap) "Keymap for custom LLM-related commands.")
...
;;   :after (gptel)
;;   :demand t
;;   :custom (gptel-prompts-directory "~/ghq/dcluna/dotfiles/gptel-prompts/")
;;   :config
...
(use-package agent-shell
  ...
  :init (let ((agent-shell-dcl-map (make-sparse-keymap)))
          (define-key agent-shell-dcl-map "r" 'agent-shell-send-region-to)
          (define-key agent-shell-dcl-map "t" 'agent-shell-open-transcript)
  ...
  (agent-shell-mcp-servers       '(((name . "sentry")
                                   ...
                                   ;; (env . (((name . "HEROKU_API_KEY")
                                   ;;          (value . (lambda ()
                                   ;;                     (require 'auth-source)
                                   ;;                     (auth-source-pick-first-password :host "heroku.com" :user "mcpserver"))))))
                                   ...
                                   ((name . "logrocket")
                                    (command . "npx")
                                    (args . ("-y" "mcp-remote@latest" "https://mcp.logrocket.com/mcp")))))
  :config
...
(define-key llm-agent-keymap "t" #'agent-shell-manager-toggle)
(use-package meta-agent-shell
  ...
  (let ((meta-agent-keymap (make-sparse-keymap)))
    (define-key llm-agent-keymap "m" #'meta-agent-shell-start)
    (define-key llm-agent-keymap "d" #'meta-agent-shell-jump-to-dispatcher)
    (define-key llm-agent-keymap "h" #'meta-agent-shell-heartbeat-start)
    (define-key llm-agent-keymap "H" #'meta-agent-shell-heartbeat-stop)
    (define-key llm-agent-keymap "s" #'meta-agent-shell-heartbeat-send-now)
