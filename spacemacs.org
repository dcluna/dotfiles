...
** Magit
   ...
   Generic toggle for magit status section hooks:
...
** Org-babel noweb/env-var conversion
   #+begin_src emacs-lisp
...
**** Agent-shell-attention installation
   #+begin_src emacs-lisp
...
** Ediff: custom scripts for merge conflict resolution

   Pressing =@= in ediff opens a prefix map for custom resolve scripts.
   Each script gets its own key under that map. Add new scripts by binding
   them in =dcl/ediff-scripts-map=.
...
** Atomic Chrome
   ...
   Edit browser text areas in Emacs via [[https://github.com/nicknisi/ghosttext-for-chrome][GhostText]] or similar extensions.
