...
** Ediff: auto-resolve structure.sql migration conflicts

   When ediff opens a =structure.sql= file, pressing =M= grabs the current
   diff region from buffers A and B, merges and sorts the migration entries
   via the Ruby script, and displays the result in =*migration-resolve*=
   for manual pasting.
   ...
   #+begin_src emacs-lisp
     ...
     Grabs the current diff region from buffers A and B, pipes combined text through
     resolve_schema_migrations.rb, and displays the result for manual pasting."
       (interactive)
     ...
     (defun dcl/ediff-maybe-add-migration-key ()
       "Add 'M' key to ediff keymap when editing structure.sql."
       (when (and ediff-buffer-A
                  (buffer-file-name ediff-buffer-A)
                  (string-match-p "structure\\.sql"
                                  (buffer-file-name ediff-buffer-A)))
         (define-key ediff-mode-map (kbd "M")
                     #'dcl/ediff-resolve-schema-migrations)))

     (add-hook 'ediff-keymap-setup-hook #'dcl/ediff-maybe-add-migration-key)
   #+end_src
